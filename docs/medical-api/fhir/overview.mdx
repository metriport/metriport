...
The `meta.source` property in any FHIR resource contains the title of the C-CDA source document from which the resource was derived. You can use this value with the[Get Document URL endpoint](/medical-api/api-reference/document/get-document) to download the original C-CDA document.
