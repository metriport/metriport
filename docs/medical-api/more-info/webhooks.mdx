...
3. Note: You will not recieve both a cxDocumentRequestMetadata and cxConsolidatedRequestMetadata in a single webhook. Below it is shown that way
   just to demonstrate the way each would look in their respective situations. cxDocumentRequestMetadata when querrying /document/query and cxConsolidatedRequestMetadata
   when querrying patient/consolidated/query
