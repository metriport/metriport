...
1. Then, if the downloaded documents contained C-CDA/XML files, when the conversion to FHIR has completed (`type` for a patient in the message will be `document-conversion`, and at this point you'll be able to [query for FHIR resrouces](/medical-api/api-reference/patient/get-patient-consolidated)).
