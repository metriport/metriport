...
1. Then, if the downloaded documents contained C-CDA/XML files, when the conversion to FHIR has completed (`type` for a patient in the message
...
When querying for a Patient's consolidated data, you can also request this information as a pdf or html.
The following is the result of the [previous section](#patient-consolidated-data), but with the `Bundle` containing a document reference to the medical record that was converted from the requested FHIR bundle.
