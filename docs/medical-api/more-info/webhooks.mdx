...
You can pass metadata to the [queried documents](/medical-api/api-reference/document/start-document-query) or [query for patient consolidated data](/medical-api/api-reference/fhir/consolidated-data-query-post),
and you will receive that data back in the meta.data field of the json. Below is an example payload within the body of your post request:

<Snippet file="webhook-metadata-post-example.mdx" />

You can specify up to 50 keys, with key names up to 40 characters long and values up to 500 characters long. This might be used to pass in your own requestID or other patient metadata.
...
```json
{
  "meta": {
    ...
    "data": {
      "<some-request-meta-property>": "<the-property-value>", 
      "<another-property>": "the other value"
    }
  },
  "patients": [
    {
