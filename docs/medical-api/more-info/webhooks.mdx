...
Instead trigger a [bulk get document url](/medical-api/api-reference/document/bulk-document-download) query - one query per patient at any given time;
When the result is ready, we’ll send a webhook message with all the downloadable urls for all the patients documents.
