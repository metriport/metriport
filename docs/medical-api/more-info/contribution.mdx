...
- All Person references must point to the Metriport patient ID the data is being upload for in this example it's `subject.reference` in the Encounter resource.
- All other references must point to a resource that's available within the same bundle.
...
- Anything that would be useful and is clinically relevant to another provider should be contributed.
