...
1. Then, either poll the API to [List Documents](/medical-api/api-reference/document/list-documents) to get the available document
   references returned from HIEs - or wait process the inbound [Webhook message](/medical-api/more-info/webhooks) which will be sent if you have Webhooks configured.
