...
<Tip>
  ...
  under the `Sandbox` section. This will allow you to try out the Medical API in Sandbox mode with
  some dummy data. Note that you'll have to generate a separate Sandbox Secret Key for this!
</Tip>
...
<Info>
  Pro-tip: you can use our handy npm package on your server if you're using Node:

```typescript
import { MetriportMedicalApi } from "@metriport/api";

const metriportClient = new MetriportMedicalApi("YOUR_API_KEY", "ENVIRONMENT_URL");
```

`ENVIRONMENT_URL` is optional and defaults to `https://api.metriport.com/medical/v1`. For sandbox mode use `https://api.sandbox.metriport.com/medical/v1`.
