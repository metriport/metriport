...
<Tip>
  You can try out the API in `Sandbox` mode anytime! This step is only for production access.
  Simply toggle the `Sandbox` switch in the Dashboard navbar. Learn more about `Sandbox` mode [here](/medical-api/more-info/sandbox).
</Tip>
