...
2. Then, either process the inbound [Webhook message](/medical-api/more-info/webhooks) which will be sent if you
...
The aforementioned FHIR resources are generated from all available CDAs for a patient, resulting in a set of unique medical records. To condense all of these records into a single document, refer to the [Medical Record Summary](/medical-api/more-info/medical-record-summary) guide.
