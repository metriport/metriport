...
This is a paginated endpoint. For more information see [pagination](/medical-api/more-info/pagination).
...
<ParamField query="filters" type="string" Optional>
  How to filter patients. It uses Metriport's [filter/search criteria](/medical-api/more-info/filters)
  ...
  - `john 2000` will return all patients with the first and/or last name equal to `john` and born in the
    year `2000`.
  - `2000-02-03` will return all patients born in `2024-02-03`.
  - `ABDC1234` will return all patients with that external ID.
...
const { patients } = await metriport.listPatients(
  facilityId, 
  filters
);
```
