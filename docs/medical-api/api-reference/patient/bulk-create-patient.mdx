...
description: "Creates multiple Patients in Metriport at once."
api: "POST /medical/v1/patient/bulk"
---

The Bulk Patient Create flow allows you to insert up to 100,000 patients at Metriport in a
single request/job. We'll also populate the medical data for those patients automatically.

This feature avoids the complexity to work within the boundaries of
[rate limits](/medical-api/more-info/limits#rate-limits) and to coordinate the data
query for large groups of patients.

<Info>
  This is an asynchronous endpoint: you upload a file with
  patient demographics and we will notify you when the
  process is completed, through a [webhook
...
with index `2` will be required (otherwise, the respective row/Patient will be marked as invalid).
...
(CSV). It migh set the status of the job to `failed` and send a corresponding webhook message.
...
Once the CSV file is uploaded, the Metriport API will start processing it and will send
[webhook messages](/medical-api/handling-data/webhooks#bulk-patient-create) to your App
indicating the status of the processing, including the resulting CSV file containing the
newly created Metriport IDs for the successfully created Patients.

#### Bulk Patient Create Status

During the processing of a bulk patient create, the job can change status following the logic
below:
...
---
flowchart LR
    A["waiting"] --> B["processing"] & C["failed"] & D["expired"]
    B --> E["completed"]
```

- `waiting`: initial state, waiting for the CSV file;
- `processing`: the CSV file has been uploaded and is being processed;
- `completed`: the bulk import job is completed;
- `failed`: the bulk import job failed (likely due to an uploaded file with invalid format);
...
<ParamField query="facilityId" type="string">
  ...
  All patients in a single job/CSV will be associated with
  the same Facility. It can be omitted if there's only one
  Facility in your account.
</ParamField>

<ParamField query="dryRun" type="boolean">
  When set to `true`, no Patients will be created, and this
  job is only used to validate the CSV file and provide
...
<ResponseField name="jobId" type="string" required>
  The ID that represents this bulk patient create job.
</ResponseField>

<ResponseField name="status" type="string" required>
  The status of the job. Always `waiting`. See all the
  possible values [here](#bulk-patient-create-status).
</ResponseField>
...
```json
{
  "jobId": "00000000-00000000-00000000-00000000",
  "status": "waiting",
