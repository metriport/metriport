...
(CSV). It might set the status of the job to `failed` and send a corresponding webhook message.
...
data is available, we send a new webhook message indicating the job is done, including
