...
<ParamField query="dataSource" type="string" optional>
  The origin of the resource. Can be one of `metriport`, `other`, or `both`. `metriport` indicates
  resources created internally by Metriport, while `other` denotes resources from external
  organizations. If not provided, will default to `both`.
</ParamField>
