---
title: "Start Bulk Get Document URL "
description: "Triggers a process to a generate a list of download URLs for all of the patient's documents."
api: "POST /medical/v1/document/bulk-download-url"
---

The status of the process is returned in the response. Initially it will be processing, and when the process is finished, the status will be updated to completed or failed.

<Tip>
  The result of the process will be delivered via webhook message - to set this up, and see the message format, check out our [webhooks guide](/medical-api/more-info/webhooks#bulk-downladable-url-generation). 
</Tip>

<Warning>
  If you were to trigger this endpoint again while the process is still running, you will get a response that reflects the current progress. So essentially, only a single bulk download process will be running for a patient at any given time.
</Warning>

## Query Params

<ParamField query="patientId" type="string" required>
  The ID of the Patient for which to initiate the bulk url generation.
</ParamField>

## Response

<ResponseField name="status" type="string" required>
      The status of the url generation process`.
</ResponseField>
<Snippet file="request-id.mdx" />

```json
{
    "status": "processing"
    "requestId": "018c1e9d-dfce-70cb-8c0e-edfbbd2a7f5f"
}
```

<ResponseExample>
```javascript Metriport SDK
import { MetriportMedicalApi } from "@metriport/api-sdk";

const metriportClient = new MetriportMedicalApi("YOUR_API_KEY");


const status = await api.startBulkGetDocumentUrl(
  "<your-patient-id>",
);

```
</ResponseExample>