...
As the query is processing, if you were to trigger this endpoint again, you will receive that the query status is `processing` as well as the current progress of documents being queried.

When the asynchronous document query finishes, it stores new/updated document references for future requests and
update the status of document query to `completed` - accessible through
