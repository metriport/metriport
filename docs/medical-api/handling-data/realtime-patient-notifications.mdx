...
Each real-time notification we send via webhook has a payload that includes key information about the event, and a url field to JSON containing robust clinical data for the encounter.
...
To use this feature, [set it up in one or more Cohorts](/medical-api/handling-data/cohorts) and associate the patients you want to have notifications enabled.
