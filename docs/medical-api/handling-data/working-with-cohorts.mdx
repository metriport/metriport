...
```json
{
  "adtMonitoring": true,
  "labMonitoring": true,
  "medicationMonitoring": true,
  "clinicalDocsMonitoring": true
}
```

<Info>
  ADT monitoring relies on the registered address of your patient - patients with `adtMonitoring: true` will receive ADTs from the state of their registered address, and _only_ that state.
</Info>
