...
We're actively building out additional monitoring capabilities. Today, these are the valid fields and values for cohort settings.

```json
{
  "adtMonitoring": true,
  "questMonitoring": true,
  "adtMonitoring_overrideHealthConnectTexas": true,
  "adtMonitoring_overrideTexasHiePcc": true
}
```

<Info>
  {" "}
  <strong>Default behavior:</strong> With <code>
    adtMonitoring: true
  </code>, a patient receives ADTs <em>only</em> from the state
  of their registered address. <br /> <strong>
    Override behavior:
  </strong> Set <code>
    adtMonitoring_overrideHieName: true
  </code> to bypass state-based routing for that HIE. For example,
  if <code>adtMonitoring_overrideTexasHiePcc</code> is <code>
    true
  </code>, we will <em>not</em> use the patient’s address to
  decide for Texas HIE PCC. You can enable multiple overrides.{" "}
</Info>
