...
- One attachment contains the main data file (JSON, HTML, or PDF).
- For JSON requests only, an additional attachment with `contentType: "application/gzip"` provides a gzip-compressed copy for faster downloads.

Each attachment has a `url` you can use to download the data.
