...
You can expect to get one or two messages of this type for each Bulk Patient Create job, with the status
...
<ResponseField name="job" type="BulkPatientCreateJob" required>
  The information about the Bulk Patient Create job.

  <Expandable title="BulkPatientCreateJob properties">
    <ResponseField name="jobId" type="string" required>
      The ID that represents this bulk patient create job.
    </ResponseField>
  
    <ResponseField name="status" type="string" required>
      The status of the job. One of `processing`, `failed`, or `completed`.
      See all the possible values
      [here](/medical-api/api-reference/patient/bulk-create-patient#bulk-patient-create-status).
    </ResponseField>

    <ResponseField name="reason" type="string">
      The reason for a failed job. Only present if the status
      is `failed`.
    </ResponseField>

    <ResponseField name="result" type="string">
      The URL to download the CSV file containing the result of
      the Bulk Patient Create job. It's valid for 5 minutes.
    </ResponseField>
...
field, which you can use to download the result of the Bulk Patient Create job.
...
<ParamField body="metriportId" type="string">
  ...
  Patiend was created successfully.
</ParamField>
...
<ParamField body="reason" type="string">
  ...
  reason for failure - usually related to parsing the
