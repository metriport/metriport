...
- The first attachment contains the main data file (JSON, HTML, or PDF format)
- For JSON requests, a second attachment with `contentType: "application/gzip"` provides a gzip-compressed version for faster downloads

Each attachment has a `url` which can be used to download the data.
...
When processing the webhook payload, identify attachments by their `contentType`:

- **Main attachment**: Look for `contentType` matching your requested format (`application/json`, `text/html`, or `application/pdf`)
- **Gzip attachment**: Look for `contentType: "application/gzip"` (JSON requests only)

Example code:

```javascript
const content = bundle.entry[0].resource.content;
...
<ResponseField name="patients" type="PatientConsolidatedData[]" required>
  ...
  <Expandable title="PatientConsolidatedData properties">
    ...
    <ResponseField name="bundle" type="Bundle">
      ...
      Each attachment has a `url` which is valid for 3 minutes.
    </ResponseField>
