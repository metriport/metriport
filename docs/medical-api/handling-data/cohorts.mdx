...
<ResponseField name="hie" type="object" optional>
  HIE-based document monitoring - subscribes patients to HIE-based document monitoring (non-ADT) 
  / medical record refresh based on the cadence.
  <Expandable title="hie object properties" defaultOpen={false}>
    <ResponseField name="enabled" type="boolean" optional>
      Enable or disable scheduled data pulls for HIE monitoring. When `false`, 
      no scheduled pulls will occur regardless of the frequency setting.
    </ResponseField>
  ...
  <Info>
    **HIE monitoring is coming soon!** While this feature is not yet live, 
    you can configure your HIE monitoring settings now. These settings will automatically 
    take effect when the feature launches.
  </Info>
</ResponseField>
...
<ResponseField name="adt" type="boolean" optional>
  ADT (Admission, Discharge, Transfer) monitoring - subscribes patients to ADT notifications
   based on their [primary address state](/medical-api/api-reference/patient/create-patient#param-is-primary).
</ResponseField>
...
<Info>
  For **pharmacy** and **laboratory** monitoring, choose either `notifications` OR `schedule`, 
  but not both. If `notifications` is `true`, `schedule` value is ignored.
</Info>
