...
- LOW RISK ( 🟢 ): medical record refresh  
- MEDIUM RISK( 🟡 ): medical record refresh & prescription medication history  
- HIGH RISK ( 🔴 ): monitoring enabled for all available sources  
...
- **`monitoring.adt`** — _allowed values: `true` | `false` | `["HieName", ...]`, default: false_  
...
- **`monitoring.pharmacy`** — _allowed values: `true` | `false`, default: false_  
  Enables **pharmacy / medication history** monitoring (e.g., retail fill data) where available.

- **`monitoring.laboratory`** - _allowed values: `true` | `false`, default: false_  
