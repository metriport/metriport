...
- 🟡 Medium Risk: `biweekly` medical record refresh & `pharmacy` notifications  
- 🔴 High Risk: `weekly` medical record refresh & monitoring enabled for all available sources

<Info>
  If you do not have **pharmacy**, **ADT**, or **laboratory** monitoring features enabled for your account, 
  these settings will be disabled in the default cohorts. Contact us if you'd like to enable these features.
</Info>
...
<Note>
  ...
  cohort](/medical-api/api-reference/patient/add-patients-to-cohort)).
</Note>
...
- [Add Patients To Cohort](/medical-api/api-reference/patient/add-patients-to-cohort)
- [Remove Patients From Cohort](/medical-api/api-reference/patient/remove-patients-from-cohort)
...
If a patient is in multiple cohorts, an enabled setting stays enabled. Even if another cohort disables it.
