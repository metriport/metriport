...
This is a guide to help you get started with iOS, specifically Apple HealthKit. First and foremost Apple HealthKit is a central repository for health and fitness data only available on iPhone and Apple Watch.
Where Metriport comes in is we provide an SDK that allows you to be able to receive Apple HealthKit data in the background.
When data gets added to Apple HealthKit by a user it will simply send that data to the Metriport api and from there it gets sent to your webhook url.

<Info>
  Apple HealthKit is available for devices on iOS v13 or higher. This also
  applies for when developing on a simulator.
</Info>
...
<Info>
  To understand the data you should expect to receive from Apple HealthKit
  checkout our [data models](/more-info/models)
</Info>
