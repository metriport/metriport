...
<Tip>
  To demo the app without having to use your production enviroment simply head over to the settings
  page and toggle "sandbox". This will allow you to try out metriport with up to 10 connected users.
</Tip>
...
<Info>
  ...
  Keys](/more-info/api-keys) page to learn more about API key security best practices.
</Info>

<Tip>
  If you believe your key has been compromised, you can always revoke it and then generate a new one
  on the Settings page by clicking the trash button beside the key.
</Tip>
...
<Info>
  ENVIRONMENT_URL is optional and defaults to https://api.metriport.com. For sandbox mode add
...
Head to the [webhook page](/more-info/webhooks) to learn how to setup this integration with Metriport.
...
<Info>
  ...
  ```typescript `https://connect.metriport.com/?token=$
  {connectToken}&sandbox=true` ```
</Info>
...
Head to the [Connect Widget page](/getting-started/connect-widget) to learn more about the various Widget customization options, and view some code samples for integrating the Widget on different platforms such as iOS and Android.
...
<Card title="API reference" icon="terminal" href="/user/create-user">
