...
<ResponseField name="genderAtBirth" type="string" required>
  The Patient's gender at birth, can be one of `M` or `F` or `UN` or `UNK`. Use `UN` when the patients gender is known but it is not `M` or `F`, i.e intersex or hermaphroditic. 
  Use UNK when the patients gender is not known.
</ResponseField>
