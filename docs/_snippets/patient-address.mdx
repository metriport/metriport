  <Expandable title="Address properties">
    <ParamField body="addressLine1" type="string" required>
      The address.
    </ParamField>

    <ParamField body="addressLine2" type="string">
      The address details, for example: `#4451`.
    </ParamField>

    <ParamField body="city" type="string" required>
      The city.
    </ParamField>

    <ParamField body="state" type="string" required>
      The 2 letter state acronym, for example: `CA`.
    </ParamField>

    <ParamField body="zip" type="string" required>
      5 digit zip code.
    </ParamField>

    <ParamField body="country" type="string">
      If specified, must be "USA"; otherwise will default to "USA".
    </ParamField>

    <ParamField body="isPrimary" type="boolean">
      If specified, marks this address as the primary address for the patient. Only one address can be marked as primary. Default will be the first address in the array.
      The primary address is used when external systems (like state HIEs) request the patient's location and need a single address.
    </ParamField>

  </Expandable>
