...
<Info>
  ...
  example, the following inputs would be equivalent: `"John,Jonathan"` & `["John", "Jonathan"]` &
  `"John Jonathan"`
</Info>

<ParamField body="firstName" type="string[] | string" required>
  The Patient's first name(s).
</ParamField>

<ParamField body="lastName" type="string[] | string" required>
