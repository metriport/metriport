...
1. **I'm following the steps to create a subscription to get access to the Devices API, but I don't see the button to create a subscription in the settings page.**
...
2. **How is rate limiting currently configured with the Devices API?**

   The limit is currently 500 requests per day.

### General

1.  **What happens when you use the same provider account with multiple connected users?**
...
2.  **How do I set this up with my database? Shouldn't I specify which database I want the data to be stored?**
...
3.  **Is any data stored on your database in the cloud managed solution of the Devices API?**
...
4.  **Is there any particular reason why the key `samples` exists?**
...
5.  **How often can I expect Webhook requests to come through?**
...
6.  **What timezone can we expect the queries to assume when the data becomes available?**

    For simplicity, all queries assume the UTC timezone for the specified date starting at 00:00 to 23:59.

7.  **Do you provide any timestamps with the response data?**
...
8.  **Are there any solutions to integrate with Metriport using Python, rather than JavaScript?**
...
1. **For Apple Health, will workouts and sleep always come with the timestamps?**
...
2. **Do Withings Webhooks events come in real time?**
...
3. **Where can I find an example of a Webhook body for Withings?**
...
4. **When I try to access data from Apple Health or Garmin, everything is returning empty. What could be the reason for this?**
...
5. **For which events do you send callbacks for Apple Health? Is it daily based?**

   Callbacks are hourly based, and for any new data that's been added to Apple Health.

6. **How much historical data can I get from Apple Health and Garmin, since these providers are Webhook based?**

   Currently, we have it configured for only 30 days of historical data for these providers, so the payload isn't too large.

7. **At what point does the app trigger a fetch from Apple Health and send data to Metirport?**

   We do this as soon as new data becomes available from Apple Health.

### Troubleshooting

1. **Why am I getting an "Oops something went wrong” error?**
...
2. **I'm trying to make a request but am getting the "forbidden" message. I used the Secret Key that I found in my developer dashboard and "1" as my `user_id`.**
...
3. **Under `users`, I see the key `sleep`. Will this change based on the data type? If so, do you have a list of all the possible values this key could be?**
