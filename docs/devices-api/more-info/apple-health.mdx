...
<Info>
  When listening for hourly changes you'll notice that the metadata contains an `hour` field. This
  is to signify the hour of the day in which the data is aggregated as shown in the [
  model](/devices-api/more-info/models#metadata).
</Info>
