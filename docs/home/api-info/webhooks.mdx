...
Here's an example of how you can do this in Node.js - see the full sample code [here](https://github.com/metriport/metriport/blob/develop/samples/typescript-express/mock-webhook.ts):
...
# Example usage
key = 'your_secret_key'  # Webhook key from the Settings page
message = '{"webhookUrl": "https://api.app.com/webhook"}'  # The message from the request in string format
signature = request.headers.get('x-metriport-signature')  # The signature from the header
