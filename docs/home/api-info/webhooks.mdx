...
- [user data](#devices-api): health information grouped by user (Devices API only).
- [patient data](#medical-api): patient document information (Medical API only).
...
<Warning>
  Currently, the Metriport doesn't implement automatic retries - let us know if this is something
  you need.
</Warning>
