...
export function formatPeriod(
  ...
  return (
    formatDateTime(period.start, locales, options) +
    " - " +
    formatDateTime(period.end, locales, options)
  );
}
...
export function formatObservationValue(
  obs: Observation | ObservationComponent | undefined
): string {
  if (!obs) {
    return "";
  }
  ...
  if ("component" in obs) {
    return (obs.component as ObservationComponent[])
      .map(c => formatObservationValue(c))
      .join(" / ");
  }
