 ...
 */
export function parseSearchDefinition<T extends Resource = Resource>(
  url: string
): SearchRequest<T> {
...
function parseUnknownParameter(
  searchRequest: SearchRequest,
  code: string,
  modifier: string,
  value: string
...
function formatFilter(filter: Filter): string {
  ...
  const prefix =
    filter.operator !== Operator.EQUALS && filter.operator in PREFIX_OPERATORS
      ? filter.operator
      : "";
