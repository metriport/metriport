...
export function findObservationInterval(
  ...
  return definition.qualifiedInterval?.find(
    interval =>
      ...
      observationIntervalMatchesValue(
        interval,
        value,
        definition.quantitativeDetails?.decimalPrecision
      ) &&
...
export function findObservationReferenceRange(
  ...
  return definition.qualifiedInterval?.find(
    interval =>
      observationIntervalMatchesPatient(interval, patient) &&
      names.includes(interval.condition as string)
  );
}
...
function observationIntervalMatchesPatient(
  ...
  return (
    observationIntervalMatchesGender(interval, patient) &&
    observationIntervalMatchesAge(interval, patient)
  );
}
 ...
 * @returns True if the patient gender matches the observation interval.
 */
function observationIntervalMatchesGender(
  interval: ObservationDefinitionQualifiedInterval,
  patient: Patient
): boolean {
 ...
 * @returns True if the patient age matches the observation interval.
 */
function observationIntervalMatchesAge(
  interval: ObservationDefinitionQualifiedInterval,
  patient: Patient
): boolean {
  return (
    !interval.age || matchesRange(calculateAge(patient.birthDate as string).years, interval.age)
  );
}
...
export function matchesRange(value: number, range: Range, precision?: number): boolean {
  return (
    (range.low?.value === undefined ||
      preciseGreaterThanOrEquals(value, range.low.value, precision)) &&
