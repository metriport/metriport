...
export class MedplumClient extends EventTarget {
  ...
  async signInWithRedirect(
    loginParams?: Partial<BaseLoginRequest>
  ): Promise<ProfileResource | void> {
  ...
  async signInWithExternalAuth(
    ...
    window.location.assign(
      this.getExternalAuthRedirectUri(authorizeUrl, clientId, redirectUri, loginRequest)
    );
  }
  ...
  async createResourceIfNoneExist<T extends Resource>(resource: T, query: string): Promise<T> {
    return ((await this.searchOne(resource.resourceType, query)) ??
      this.createResource(resource)) as Promise<T>;
  }
  ...
  executeBot(
    ...
    if (typeof idOrIdentifier === "string") {
      ...
      url =
        this.fhirUrl("Bot", "$execute") + `?identifier=${identifier.system}|${identifier.value}`;
    }
  ...
  readResourceGraph<K extends ResourceType>(
    ...
    return this.get<Bundle<Resource>>(
      `${this.fhirUrl(resourceType, id)}/$graph?graph=${graphName}`
    );
  }
  ...
  #addLogin(newLogin: LoginState): void {
    const logins = this.getLogins().filter(
      login => login.profile?.reference !== newLogin.profile?.reference
    );
  ...
  async #executeAutoBatch(): Promise<void> {
    ...
    if (entries.length === 1) {
      ...
      try {
        entry.resolve(
          await this.#request(entry.method, this.#fhirBaseUrl + entry.url, entry.options)
        );
      } catch (err) {
      ...
      entry: entries.map(
        e =>
          ({
            ...
            resource: e.options.body
              ? (JSON.parse(e.options.body as string) as Resource)
              : undefined,
          } as BundleEntry)
      ),
    };
