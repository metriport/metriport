...
export const mapToSleep = (
  ...
  if (withingsSleep.length) {
    ...
    sleep.start_time = dayjs(mainSleep.startdate).format(
      "YYYY-MM-DDTHH:mm:ssZ"
    );
    sleep.end_time = dayjs(mainSleep.enddate).format("YYYY-MM-DDTHH:mm:ssZ");

    if (mainSleep.data) {
      const { total_sleep_time, wakeupduration, deepsleepduration, remsleepduration, lightsleepduration, total_timeinbed } = mainSleep.data;
