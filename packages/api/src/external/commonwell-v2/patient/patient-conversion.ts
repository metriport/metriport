...
export function getCwStrongIdsFromPatient(patient: Patient): PatientIdentifier[] {
  return (patient.data.personalIdentifiers ?? []).flatMap(id => {
    ...
    if (id.type === "driversLicense") {
      return { ...base, value: id.value, system: driversLicenseURIs[id.state] };
    }
    const system = identifierSytemByType[id.type];
    if (!system) return [];
    return { ...base, value: id.value, system };
  });
}
