...
import { RequestMetadata } from "@metriport/commonwell-sdk-v1";
...
import {
  fhirIdentifierToDriversLicense,
  mapMetriportGenderToFhirGender,
} from "@metriport/core/external/fhir/patient/conversion";
...
import { CwLink, CwLinkV2 } from "../../commonwell/patient/cw-patient-data/shared";
 ...
 *
 */
export async function registerPatientInCwV2({
  patient,
  facilityId,
  getOrgIdExcludeList,
...
export async function updatePatientAndLinksInCwV2({
  ...
  try {
    ...
    // Note: networkLinks is NetworkLink[] from v1, but assessProbableLinks expects PatientCollection from v2
    // For now, we'll skip the assessment since this is a v2 function and networkLinks is v1 format
    const cwLinks: CwLink[] = [];
...
async function setupApiAndCwPatient({
  ...
  const commonWellAPI = makeCommonWellAPI(initiatorName, addOidPrefix(initiatorOid), initiatorNpi);
...
//   const { log, debug } = out(`CW.v2 findOrCreatePersonAndLink - CW patientId ${commonwellPatientId}`);
...
// TODO: This function needs to be implemented for v2 API compatibility
// The v2 API has different methods and signatures than v1
async function updatePersonAndLink({
  ...
  // TODO: Implement v2 version of this function
  return undefined;
}
...
async function registerPatient({
  ...
  const commonwellPatientId = getPatientIdFromCollectionItem(respPatient);
...
async function updatePatient({
  ...
  const { log, debug } = out(`CW.v2 updatePatient - CW patientId ${commonwellPatientId}`);
...
//   const { debug } = out(`CW.v2 getLinkInfo - CW patientId ${commonwellPatientId}`);
...
export async function autoUpgradeProbableLinks({
  commonWell,
  validLinks,
  invalidLinks,
  commonwellPatientId,
  ...
  // debug(`existingLinks: `, JSON.stringify(existingLinks));
...
function probableLinkToPatientData(
  patientItem: PatientExistingLink | PatientProbableLink
): PatientData {
...
function patientCollectionItemToCwLinkV2(patientItem: PatientResponseItem): CwLinkV2 {
  return {
    ...patientItem,
    version: 2,
  };
