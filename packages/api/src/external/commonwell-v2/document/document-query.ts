...
export async function internalGetDocuments({
  ...
  const commonWell = makeCommonWellAPI(initiator.name, addOidPrefix(initiator.oid), initiator.npi);
  ...
  try {
    ...
    for (const item of queryResponse.entry ?? []) {
      if (item.resource?.resourceType === documentReferenceResourceType) {
        docs.push(item as CwDocumentReference);
...
function convertToNonExistingS3Info(
  ...
  return async (doc: DocumentWithMetriportId): Promise<S3Info> => {
    ...
    const simpleFile = await docToFile({
      id: doc.id,
      contentType: doc.content?.[0]?.attachment.contentType ?? undefined,
    });
    return {
      ...simpleFile,
      fileExists: false,
