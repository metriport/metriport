...
import { AuthorTypes, createDocReferenceContent, getFHIRDocRef } from "../../fhir/document";
...
function cwStatusToFHIR(status: DocumentContent["status"]): DocumentReference["status"] {
  switch (status) {
    case "current":
      return "current";
...
function idToFHIR(id: DocumentIdentifier): Identifier {
  return {
    system: id.system,
    value: id.value,
...
function containedPatientToFHIRResource(
...
function containedPractitionerToFHIRResource(
  resource: Contained,
  patientId: string,
  log: (msg: string) => void
): Resource[] | undefined {
