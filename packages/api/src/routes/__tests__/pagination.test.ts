...
describe("Pagination", () => {
  describe("paginated", () => {
    ...
    async function getItems(pagination: Pagination) {
      let items = [...mockItems];
    ...
    describe("validation", () => {
      it("rejects when both fromItem and toItem provided", () => {
        ...
        expect(() => paginated(req as any, getItems)).rejects.toThrow(
      ...
      it("accepts valid count parameter", async () => {
        ...
        const result = await paginated(req as any, getItems);
    ...
    describe("logic", () => {
      it("returns empty array when no items exist", async () => {
        ...
        const result = await paginated(req as any, emptyGetItems);
      ...
      it("returns single item when only one exists", async () => {
        ...
        const req = { query: {}, baseUrl };
        const result = await paginated(req as any, singleItemGetItems);
      ...
      it("defaults to 50 items per page", async () => {
        ...
        const req = { query: {}, baseUrl };
        const result = await paginated(req as any, moreItemsThanDefaultCount);
      ...
      it("returns first page when no pagination params provided", async () => {
        ...
        const req = { query: {}, baseUrl };
        const result = await paginated(req as any, hundredPlusItems);
        expect(result.items).toHaveLength(defaultItemsPerPage);
        expect(result.items[0].id).toBe("1");
        expect(result.items[1].id).toBe("2");
