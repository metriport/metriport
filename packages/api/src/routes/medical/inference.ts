...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    log(`Input (${context.length} chars):\n\n${JSON.stringify(resources)}\n`);
    ...
    const { message, inputTokens, outputTokens } = await askGroq({
      context,
      resourceType,
      resourceDisplays,
    ...
    }).then(({ message, inputTokens, outputTokens }) => {
      console.log(
...
async function askGroq({
  ...
  try {
    ...
    return { message: undefined, inputTokens: undefined, outputTokens: undefined };
  }
}
