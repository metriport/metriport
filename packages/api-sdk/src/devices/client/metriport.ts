...
export class MetriportDevicesApi {
  ...
  /**
   * Connects the user to a remote patient monitoring (RPM) device provider (currently only includes Tenovi).
   *
   * @param {string} provider - A healthcare data provider.
   * @param {string} state - The connect token that was generated by the getConnectToken method.
   * @param {string} deviceIds - A comma-separated list of device IDs to connect to the user.
  ...
  async connectRpmProvider(
    ...
    await this.api.post(
      `/connect/rpm/:provider=${provider}/state?=${state}&device_id=${deviceIds}&device_user_id=${deviceUserId}`
    );
  }
