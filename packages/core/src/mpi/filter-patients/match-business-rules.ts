...
export function checkBusinessRules(
...
): { isMatch: false; totalScore: 0; scores: { [key: string]: number }; failedRule: string } | null {
  // Rule: If first name and last name don't match at all, patient should not match
  if (scores.names === 0) {
    return { isMatch: false, totalScore: 0, scores, failedRule: "No Name Match" };
  }
  ...
  if (isLastNameAndAddressMismatch(metriportPatient, externalPatient, scores)) {
    return {
      ...
      totalScore: 0,
      scores,
      failedRule: "Last Name Wrong + Address Incorrect",
    };
  }
  ...
  if (scores.dob <= 1 && scores.address < 2) {
    return {
      ...
      totalScore: 0,
      scores,
      failedRule: "DOB 2+ Parts Wrong + Address Not Same",
    };
  }
  ...
  if (scores.dob === 2 && scores.address < 2) {
    if (hasNoContact(scores)) {
      return {
        ...
        totalScore: 0,
        scores,
        failedRule: "DOB 1 Part Wrong + Address Not Perfect + No Contact Match",
      };
    }
  }
  ...
  if (scores.dob === 0 && isDobOffByMoreThan15Years(metriportPatient, externalPatient)) {
    return {
      ...
      totalScore: 0,
      scores,
      failedRule: "DOB Off By More Than 15 Years + No Parts Match",
    };
  }
  ...
  if (isNameAddressAndContactMismatch(scores)) {
    return {
      ...
      totalScore: 0,
      scores,
      failedRule: "Name + Address + Contact All Mismatch",
    };
  }
