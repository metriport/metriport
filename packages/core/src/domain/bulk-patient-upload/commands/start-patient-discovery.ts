...
export async function startPatientDiscovery({
  ...
  const { log, debug } = out(
    `BulkUpload start patient discovery - cxId ${cxId} patientId ${patientId}`
  );
  ...
  try {
    ...
    const msg = `Failure while starting patient discovery @ BulkUpload`;
    ...
    capture.error(msg, {
      extra: {
        ...
        cxId,
        patientId,
        context: "bulk-upload.start-patient-discovery",
        error,
      },
    });
    throw error;
