import { JwtTokenInfo, AdditionalInfo, MetriportError, errorToString } from "@metriport/shared";
...
function buildS3Path(ehr: string, method: string, key: string): string {
  return `${ehr}/${method}/${key}/${uuidv7()}.json`;
}
...
export async function makeRequest<T>({
  ...
  if (responsesBucket && s3Utils) {
    ...
    const key = buildS3Path(ehr, method, filePath);
    s3Utils
      .uploadFile({
