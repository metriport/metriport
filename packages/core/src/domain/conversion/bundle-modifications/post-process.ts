...
export function postProcessBundle(
  ...
  // It's important to add extensions before replacing IDs b/c currently replaceIdsForResourcesWithDocExtension only replaces IDs of resources with extensions (#2574 to fix that).
  ...
  const withNewIds = replaceIdsForResourcesWithDocExtension(
    withExtensions,
    patientId,
    documentExtension
  );
