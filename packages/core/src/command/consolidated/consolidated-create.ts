...
function insertSourceDocumentToAllDocRefMeta(docRefs: DocumentReference[]): DocumentReference[] {
...
export function insertSourceDocumentToDocRefMeta<T extends DocumentReference>(docRef: T): T {
  const metriportContent = docRef.content?.find(c => c.extension?.find(isMetriportExtension));
