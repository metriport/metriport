...
describe("FHIR converter", () => {
  ...
  it("should extract a medication with multiple frequency attributes", async () => {
    testArtifact("amoxicillin-infection");
  });

  it("should extract a dosage and frequency for a medication", async () => {
    testArtifact("dosage-frequency");
  });

  /**
   * > Administer Heparin 1000 units/mL injection, 5,000 units subcutaneously in the left thigh every 12 hours at a rate of 1 mL/min.
   */
  it("should extract a medication with multiple routes and modes", async () => {
    testArtifact("heparin");
  });

  /**
   * > Apply 2 grams of Hydrocortisone cream to affected area twice daily.
   */
  it("should extract a medication with multiple strengths", async () => {
    testArtifact("hydrocortisone");
  });

  it("should extract a medication with multiple forms", async () => {
    testArtifact("metform");
  });

  it("should handle negation traits", async () => {
    testArtifact("negation-trait");
  });

  /**
   * > Prednisone 40 mg tablet, take one tablet by mouth daily for 5 days, then taper by 10 mg every 3 days until discontinued.
   */
  it("should handle a complex timing case", async () => {
    testArtifact("prednisone");
  });
});
