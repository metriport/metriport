...
import { normalizeEmailNewSafe } from "../contact/email";
...
export const patientImportPatientSchema = z.object({
  ...
  email1: z.string().refine(normalizeEmailNewSafe, { message: "Invalid email" }).optional(),
  email2: z.string().refine(normalizeEmailNewSafe, { message: "Invalid email" }).optional(),
