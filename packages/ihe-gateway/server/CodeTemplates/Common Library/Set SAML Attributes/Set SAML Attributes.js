...
function setSAMLAttributes(msg) {
	...
	if (msg.samlAttributes.hasOwnProperty('organization') && msg.samlAttributes.organization.toString().length > 0) try {
	...
	channelMap.put('HomeCommunityId', configurationMap.get('SAML.HomeCommunityId'));
	if (msg.samlAttributes.hasOwnProperty('homeCommunityId') && msg.samlAttributes.homeCommunityId.toString().length > 0) try {
		var homeCommunityId = (msg.samlAttributes.homeCommunityId.toString().startsWith('urn:oid:')) ? msg.samlAttributes.homeCommunityId.toString().replace('urn:oid:','') : msg.samlAttributes.homeCommunityId.toString();
	...
	if (msg.samlAttributes.hasOwnProperty('purposeOfUse') && msg.samlAttributes.purposeOfUse.toString().length > 0) try {
	...
	if (msg.samlAttributes.hasOwnProperty('subjectRole') && 
		msg.samlAttributes.subjectRole.code.toString().length > 0 && msg.samlAttributes.subjectRole.display.toString().length > 0) try {
