...
function xcaWriteToFile(path, document, metadata) {
	...
	try {
		...
		var putRequest = Packages.software.amazon.awssdk.services.s3.model.PutObjectRequest.builder().bucket(bucketName).key(path.toString()).metadata(meta).build();
		var requestBody = Packages.software.amazon.awssdk.core.sync.RequestBody.fromBytes(java.lang.String(document.toString()).getBytes());
