...
import { IHEGatewayV2XCPDRequestParams } from "@metriport/core/external/carequality/ihe-gateway-v2/invoke-patient-discovery";
...
export const handler = Sentry.AWSLambda.wrapHandler(
  async (event: IHEGatewayV2XCPDRequestParams) => {
    try {
      const { patientId, cxId, pdRequestIHEGatewayV2 } = event;
      ...
      // validate request
      const xcpdRequest = outboundPatientDiscoveryReqSchema.safeParse(pdRequestIHEGatewayV2);
