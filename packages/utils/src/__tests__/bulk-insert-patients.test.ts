...
import fs from "fs";
import { normalizeAddressLine } from "../bulk-insert-patients";

describe("bulk-insert-patients", () => {
  describe("normalizeAddressLine", () => {
    ...
    describe("redacted from the wild", () => {
      describe("expected", () => {
        ...
        /**
         * As we update these, make sure to:
         * 1. set "fileName" above to the file that contains data we want to prevent on the repository
         * 2. run the surrounding "describe"
         * 3. once all tests pass, remove the file name from the `fileName` constant
         * 4. commit and push
         */
...
Apt 1B`,
        ...
        if (fileName) {
          it("does not contain sensitive information", done => {
            expect.assertions(1);
            fs.createReadStream(fileName)
              .pipe(csv())
              .on("data", async data => {
                const dataStr = JSON.stringify(data);
                if (addresses.some(a => dataStr.includes(a.input))) {
                  console.log(">>>> FOUND SENSITIVE DATA!!!!! " + dataStr);
                  throw new Error("Sensitive data found");
                }
              })
              .on("end", async () => {
                console.log(">>>> done");
