 ...
 * and outputs JSON payloads for TCM encounter creation / Discharge summary investigation.
 *
 * Usage:
...
const LOOKBACK = { amount: 24, unit: "hours" as const };
...
async function main() {
  ...
  const { log } = out("collect-tcm-encounter");
  ...
  const cutoffDate = dayjs().subtract(LOOKBACK.amount, LOOKBACK.unit);

  // Initialize output folder
  initRunsFolder();
  const timestamp = dayjs().toISOString();
  const outputFolder = `runs/collect-tcm-encounter/${timestamp}`;
...
export function convertToTcmEncounterPayload(
  ...
  return {
    ...
    clinicalInformation: {},
    freetextNote: `Auto-generated from ${latestEvent} event`,
  };
}
