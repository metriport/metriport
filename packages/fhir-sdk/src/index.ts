...
export class FhirBundleSdk {
  ...
  generateLLMContext<T extends Resource>(
    ...
    const walkResult = this.walkReferences(startResource, {
      maxDepth,
    ...
    for (const [depth, resources] of walkResult.resourcesByDepth.entries()) {
      ...
      cleanedResourcesByDepth.set(
        depth,
        resources.map(r => FhirBundleSdk.stripNonClinicalData(r))
      );
    }

    // Format based on selected format
    if (format === "json") {
      return this.formatAsJSON(cleanedResourcesByDepth, startResource);
    } else {
      return this.formatAsStructuredText(cleanedResourcesByDepth, startResource);
    }
  }
   ...
   */
  private formatAsJSON<T extends Resource>(
    resourcesByDepth: Map<number, Resource[]>,
    startResource: Smart<T>
  ): string {
   ...
   */
  private formatAsStructuredText<T extends Resource>(
    resourcesByDepth: Map<number, Resource[]>,
    startResource: Smart<T>
  ): string {
    const lines: string[] = [];
    ...
    if (primaryResources.length > 0) {
      ...
      for (const resource of primaryResources) {
        lines.push(this.formatResourceAsText(resource));
        lines.push("");
      }
    }
   ...
   */
  private formatResourceAsText(resource: Resource): string {
    const lines: string[] = [];
   ...
   */
  private groupResourcesByType(resources: Resource[]): Record<string, Resource[]> {
