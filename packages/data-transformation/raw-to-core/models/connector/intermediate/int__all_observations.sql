...
,coding_system as (
    select * from (
        select 
            *, 
            row_number() over(partition by id order by table_priority,code_type_priority,array_priority) as rn 
        from (
            select id,
                table_priority,
                array_priority,
...
select
    ...
    , case 
        when obvs.effectiveperiod_start != '' and obvs.effectiveperiod_start is not null then {{ try_to_cast_date('obvs.effectiveperiod_start') }}
        when obvs.effectivedatetime != '' and obvs.effectivedatetime is not null then {{ try_to_cast_date('obvs.effectiveperiod_start') }}
        else null
      end as observation_date
    , case 
        when obvs.effectiveperiod_start != '' and obvs.effectiveperiod_start is not null then {{ try_to_cast_date('obvs.effectiveperiod_start') }}
        when obvs.effectivedatetime != '' and obvs.effectivedatetime is not null then {{ try_to_cast_date('obvs.effectivedatetime') }}
        else null
      end as collection_date
    , case 
        when obvs.effectiveperiod_end != '' and obvs.effectiveperiod_end is not null then {{ try_to_cast_date('obvs.effectiveperiod_end') }}
        when obvs.effectivedatetime != '' and obvs.effectivedatetime is not null then {{ try_to_cast_date('obvs.effectivedatetime') }}
        else null
      end as result_date
