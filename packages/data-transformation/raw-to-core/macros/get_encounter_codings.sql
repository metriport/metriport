{% macro get_encounter_type_codings(stage_table, max_index, max_second_index) %}
    ...
    select 
        ...
        ,   case 
                when type_{{i}}_coding_{{j}}_system = 'http://terminology.hl7.org/CodeSystem/v3-ActCode' then 'act'
                when type_{{i}}_coding_{{j}}_system = 'http://www.ama-assn.org/go/cpt' then 'cpt'
...
{% macro get_encounter_discharge_disposition_codings(stage_table, max_index) %}
    ...
    select 
        ...
        ,   case 
                when hospitalization_dischargedisposition_coding_{{i}}_system = 'http://terminology.hl7.org/CodeSystem/v3-ActCode' then 'act'
                when hospitalization_dischargedisposition_coding_{{i}}_system = 'http://www.ama-assn.org/go/cpt' then 'cpt'
...
{% macro get_encounter_priority_codings(stage_table, max_index) %}
    ...
    select 
        ...
        ,   case 
                when priority_coding_{{i}}_system = 'http://terminology.hl7.org/CodeSystem/v3-ActCode' then 'act'
                when priority_coding_{{i}}_system = 'http://www.ama-assn.org/go/cpt' then 'cpt'
